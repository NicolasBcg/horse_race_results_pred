...
x_train = pd.read_csv(PATH + directory_encode + '/X_train.csv').to_numpy()
...
model = MLPClassifier(hidden_layer_sizes=(128, 64), activation='relu', solver='adam', alpha=0.0001, batch_size=32,
                      learning_rate_init=0.001, max_iter=50, verbose=True, random_state=42)
...
x_test = pd.read_csv(PATH + directory_encode + '/X_test.csv').values
