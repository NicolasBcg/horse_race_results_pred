...
def encode_primary(filename,date1,date2,DATASET,NORMALIZE = False):
    ...
    os.makedirs(PATH+filename+"/encoded_datas")      
    if NORMALIZE:
    # Normalize the data
    ...
    print(df.shape)
    print(df_y.shape)    
    df.to_csv(PATH+filename+"/encoded_datas/X_train.csv",index=False)
    df_y.to_csv(PATH+filename+"/encoded_datas/Y_train.csv",index=False)
...
def encode_new_data(filename,date1,date2,DATASET,training=True,NORMALIZE = False):
    ...
    if training:
        print(df_y.shape)
        df.to_csv(PATH+filename+"/encoded_datas/X_valid.csv",index=False)
        df_y.to_csv(PATH+filename+"/encoded_datas/Y_valid.csv",index=False)
    else : 

        df.to_csv(PATH+filename+"/encoded_datas/X_test.csv",index=False)
        print("load results")
        ...
        print("save results")
        resultats.to_csv(PATH+filename+"/encoded_datas/Y_test.csv",index=False)
