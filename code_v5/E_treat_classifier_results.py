...
def load_model_and_data(directory_encode,model_choice,model_extension,prefix_res=''):
    print("Loading DATAS")
    x_test = pd.read_csv(PATH + directory_encode +'/encoded_datas/'+prefix_res+'X_test.csv')
    print("Loading Model")
    if model_choice == "xgboost":
        model = joblib.load(PATH + directory_encode +"/"+directory_encode +"-xgboost-"  + model_extension+".dat")
    elif model_choice == "randomForest":
        model = joblib.load(PATH + directory_encode +"/"+directory_encode +"-randomForest-" + model_extension+".sav")
    elif model_choice == "lgbm":
        model = joblib.load(PATH + directory_encode +"/"+directory_encode +"-lgbm-"  + model_extension+ ".dat")
    elif model_choice == "logreg":
        model = joblib.load(PATH + directory_encode +"/"+directory_encode +"-logreg-"  + model_extension+ ".dat")
        x_test = pd.DataFrame(x_test).fillna(pd.DataFrame(x_test).mean())
    else:
        raise ValueError("Invalid model choice. Please select 'xgboost', 'randomForest', or 'lgbm'.")
    ...
    resultats = pd.read_csv(PATH + directory_encode +'/encoded_datas/'+prefix_res+'Y_test.csv')
...
def process_classifier(model_choice,directory_encode,model_extension,display=False,prefix_res=''):
    resultats,x_test,model,numsPMU = load_model_and_data(directory_encode,model_choice,model_extension,prefix_res)
    lines = [0]
    lines_res = [0]
    print(len(resultats))
    ...
    if ignore_fantomes:
        fantomes= "_no_F"
    final.to_csv(PATH + directory_encode + '/resultats/'+prefix_res +directory_encode +'_' +model_choice+'_borda_norm'+model_extension+fantomes+'.csv',index=False)
    if display :     
        display_prob(probs,res)  
        plotProbas(probs)
    return prefix_res +directory_encode +'_' +model_choice+'_borda_norm'+model_extension+fantomes
