...
jockeys={} #Jockey->year_semester-> nb courses, pl 1er, pl 2eme, pl 3eme, non_arrive,dernier
files=os.listdir(PATH_TO_CACHE+"programmes\\")
print("FILES :")
print(len(files))
def update_jockeys(participants,ordreArrivee):
                for participant in participants["participants"]:
                    if "driver" in participant.keys():
                        jockey=participant["driver"]
                        num_PMU=participant["numPmu"]
                        if jockey in jockeys.keys():
                            if year_semester not in jockeys[jockey].keys():
                                jockeys[jockey][year_semester]=[0,0,0,0,0,0]
                        else : 
                            jockeys[jockey]= {}
                            jockeys[jockey][year_semester]=[0,0,0,0,0,0]
                        jockeys[jockey][year_semester][0]+=1
                        if num_PMU not in ordreArrivee:
                            jockeys[jockey][year_semester][4]+=1
                        elif num_PMU == ordreArrivee[0]:
                            jockeys[jockey][year_semester][1]+=1
                        elif num_PMU == ordreArrivee[1]:
                            jockeys[jockey][year_semester][2]+=1
                        elif num_PMU == ordreArrivee[2]:
                            jockeys[jockey][year_semester][3]+=1
                        elif num_PMU == ordreArrivee[-1]:
                            jockeys[jockey][year_semester][5]+=1
        ...
        for course in reunion['courses']:
            if "ordreArrivee" in course.keys():
                ...
                try:
                    ...
                    update_jockeys(participants,ordreArrivee)
                except :
                    print(PATH_TO_CACHE+"participants\\"+date+'-'+str(num_reunion)+'-'+str(course["numOrdre"])+'.json')

with open(PATH_TO_DATASETS+"jockeys.json", "w") as json_file:
    json.dump(jockeys, json_file)
