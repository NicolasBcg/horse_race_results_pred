...
last_elos_path=PATH_TO_DATASETS+"last_elo_calculated.json"
...
if os.path.exists(last_elos_path):
    ...
    with open(last_elos_path, 'r') as file:
        ...
        try:
            ...
            print(f"last date {str(last_date)}")
            ...
            f=last_elos["last_file_calculated"]
        except : 
            print("impossible to process last date")
else:
...
def calc_elo(updated_jockeys=updated_jockeys,updated_elo_jockeys=updated_elo_jockeys,updated_elo_jockeys_v2=updated_elo_jockeys_v2):
    k=0
    for f in files:
        k+=1
        if k%100==0:
    ...
    if ENV == "DEV" :
        with open(last_elos_path, 'w') as file:
            json.dump(last_elos, file)    
